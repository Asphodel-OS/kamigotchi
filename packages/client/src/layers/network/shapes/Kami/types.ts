...
export const getKami = (
  ...
  if (kami.level) {
    ...
    const expereinceExponentArr = getConfigFieldValueArray(network, 'KAMI_LVL_REQ_MULT_BASE');
  ...
  if (kami.state === 'HARVESTING') {
    ...
    const drainBaseArr = getConfigFieldValueArray(network, 'HEALTH_RATE_DRAIN_BASE');
    ...
    const healBaseArr = getConfigFieldValueArray(network, 'HEALTH_RATE_HEAL_BASE');
