...
export const Login = (props: Props) => {
  ...
  useEffect(() => {
    const storedData = localStorage.getItem('farcaster-user');
    if (storedData) {
      const user: FarcasterUser = JSON.parse(storedData);
      console.log('mounting. user:', user);
        ...
        intervalId = setInterval(async () => {
          try {
            ...
            if (user?.status === 'approved') {
              ...
              localStorage.setItem('farcaster-user', JSON.stringify(user));
  ...
  const handleCast = async () => {
    setIsCasting(true);
    const castText = text.length === 0 ? 'test cast' : text;
    try {
    ...
    <Container key={account.name}>
      <div className='App'>
        ...
        {/* {farcasterUser?.status == 'approved' && (
              <div>
                <div className='user-info'>{`You are logged in as fid ${farcasterUser.fid}`}</div>
                <div className='cast-container'>
                  <textarea
                    className='cast-textarea'
                    placeholder={'test cast'}
                    value={text}
  ...
  async function createAndStoreSigner() {
    try {
      ...
      if (response.status === 200) {
        localStorage.setItem('farcaster-user', JSON.stringify(response.data));
