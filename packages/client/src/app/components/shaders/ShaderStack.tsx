...
export const ShaderStack: FC<ShaderStackProps> = ({
  layers,
  className,
  style,
  paused = false,
  ...
  /////////////////
  // ??? (idk what to call this section but any appropriate label applies)

  // initialize the renderer and
  const init = () => {
    const container = containerRef.current;
    ...
    const mats: ShaderMaterial[] = layers.map((layer) => {
      const mergedUniforms: Record<string, IUniform> = {
        iTime: { value: 0 },
        iResolution: { value: new Vector3(1, 1, window.devicePixelRatio || 1) },
        ...(layer.uniforms ?? {}),
      };
      const mat = new ShaderMaterial({
        ...
        transparent: true,
        blending: layer.blending ?? NormalBlending,
        depthTest: false,
        depthWrite: false,
      });
      return mat;
