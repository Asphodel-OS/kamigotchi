...
interface Props {
  ...
  actions: {
    useAccount: (item: Item, amount: number) => void;
    useKami: (kami: Kami, item: Item) => void;
  };
  utils: {
...
export const ItemGrid = (props: Props) => {
  ...
  const getKamiActions = (item: Item): Option[] => {
    ...
    return kamis.map((kami) => ({ text: kami.name, onClick: () => actions.useKami(kami, item) }));
  };
  ...
  const getAccountActions = (item: Item, bal: number): Option[] => {
    ...
    const options = [{ text: 'Use', onClick: () => actions.useAccount(item, 1) }];
    if (bal > 1)
      options.push({ text: `Use ${count}`, onClick: () => actions.useAccount(item, count) });
    return options;
  };
