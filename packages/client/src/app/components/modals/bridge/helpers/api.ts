...
/// source options whose asset the router can currently route. the router
...
export async function fetchRoutableSourceOptionIds(
  ...
  const all = new Set(options.map((o) => o.id));
  try {
    const ids = Array.from(new Set(options.map((o) => o.chainId))).join(',');
    ...
    for (const option of options) {
      ...
      const asset = assets.find((a) => a.denom?.toLowerCase() === option.denom.toLowerCase());
      if (asset && !asset.hidden) routable.add(option.id);
    }
    return routable;
  } catch {
    return all;
