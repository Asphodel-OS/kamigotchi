...
library LibEquipment {
  ...
  function equip(
    ...
    if (existingEquipID != 0) {
      ...
      require(
        getEquippedCount(components, holderID) < getCapacity(components, holderID),
  ...
  function getAllEquipped(
    ...
    return
      IDOwnsEquipmentComponent(getAddrByID(components, IDOwnsEquipCompID)).getEntitiesWithValue(
        holderID
      );
  }
