...
export async function run() {
  ...
  function setUpWorld(api: StateAPI) {
    ...
    function initConfig(api: StateAPI) {
      ...
      api.config.set.number('KAMI_RESTING_RATE', 120); // in respect to harmony
      api.config.set.number('KAMI_RESTING_RATE_PREC', 2); // i.e. x/100
    ...
    function initLocalConfig(api: StateAPI) {
      ...
      api.config.set.number('KAMI_RESTING_RATE', 10000); // in respect to harmony
    }
