...
contract _RegistryCreateLootboxSystem is System {
  ...
  function execute(bytes memory arguments) public onlyOwner returns (bytes memory) {
    (uint256 index, uint256[] memory keys, uint256[] memory weights) = abi.decode(
      arguments,
    ...
    LibRegistryItem.createLootbox(world, components, index, keys, weights);
  ...
  function executeTyped(
    ...
    return execute(abi.encode(index, keys, weights));
  }
}
