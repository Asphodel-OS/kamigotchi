...
uint256 constant ID = uint256(keccak256("system._Registry.Reward.Create"));

// real important to remove this for deployment! would allow for free minting
// gives coins to the calling account
contract _RegistryCreateRewardSystem is System {
  ...
  function execute(bytes memory arguments) public onlyOwner returns (bytes memory) {
    ...
    uint256 id = LibRegistryQuests.createEmptyReward(world, components, index, name, logicType);
