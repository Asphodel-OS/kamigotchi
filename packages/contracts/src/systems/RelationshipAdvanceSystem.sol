...
contract RelationshipAdvanceSystem is System {
  ...
  function execute(bytes memory arguments) public returns (bytes memory) {
    (uint32 npcIndex, uint256 relIndex) = abi.decode(arguments, (uint32, uint256));
  ...
  function executeTyped(uint32 npcIndex, uint256 relIndex) public onlyOwner returns (bytes memory) {
