...
contract _NPCSetRoomSystem is System {
  ...
  function execute(bytes memory arguments) public onlyOwner returns (bytes memory) {
    (uint32 index, uint256 roomIndex) = abi.decode(arguments, (uint32, uint256));
  ...
  function executeTyped(uint32 index, uint256 roomIndex) public onlyOwner returns (bytes memory) {
